  ...
  if (opts.mode == "resize") {
    ...
    for (int i = 0; i < opts.num_threads; ++i) {
      workers.emplace_back(
          std::thread(pr::treatImage, std::ref(fileQueue), std::ref(opts.outputFolder)));
    }
    pr::findImageFiles(opts.inputFolder,
                       [&](const std::filesystem::path &file) { fileQueue.push(file); });
    ...
    const int n = opts.num_threads;

    pr::FileQueue fileQueue(64);
    pr::ImageTaskQueue imageQueue(64);
    pr::ImageTaskQueue resizedQueue(64);
    ...
    for (int i = 0; i < n; ++i) {
      readers.emplace_back(std::thread(pr::reader, std::ref(fileQueue), std::ref(imageQueue)));
    }

    for (int i = 0; i < n; ++i) {
      resizers.emplace_back(std::thread(pr::resizer, std::ref(imageQueue), std::ref(resizedQueue)));
    }

    for (int i = 0; i < n; ++i) {
      savers.emplace_back(
          std::thread(pr::saver, std::ref(resizedQueue), std::ref(opts.outputFolder)));
    }
